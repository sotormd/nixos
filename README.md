...
| `init disks <boot\|swap\|root\|mount>` | ✔        | ✘        | <br>`$ nixos init disks boot` <br>Format $BOOT as FAT32. <br><br>`$ nixos init disks swap`<br>Format $SWAP as swap. <br><br>`$ nixos init disks root`<br>Create LUKS encrypted ZFS rpool on $ROOT. <br><br>`$ nixos init disks mount` <br>Mount partitions under /mnt |
