...
let
  ...
  yuck = pkgs.writeTextFile {
    ...
    text = ''
      ...
      (defpoll SONG :interval "0.5s" `${scripts}/music.sh --song`)
      (defpoll ARTIST :interval "0.5s" `${scripts}/music.sh --artist`)
      (defpoll STATUS :interval "0.5s" `${scripts}/music.sh --status`)
      (defpoll CURRENT :interval "0.5s" `${scripts}/music.sh --time`)
      (defpoll COVER :interval "0.5s" `${scripts}/music.sh --cover`)
      (defpoll CTIME :interval "0.5s" `${scripts}/music.sh --ctime`)
      (defpoll TTIME :interval "0.5s" `${scripts}/music.sh --ttime`)
