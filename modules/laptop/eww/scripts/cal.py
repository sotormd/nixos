            ...
            week.append(
                {
                    "value": value[0],
                    "today": (
                        True if today == current_date.strftime("%Y-%m-%d") else False
                    ),
...
year = subprocess.run(
    ["eww", "get", "calendar-selected-year"], stdout=subprocess.PIPE, text=True
).stdout.strip()
month = subprocess.run(
    ["eww", "get", "calendar-selected-month"], stdout=subprocess.PIPE, text=True
).stdout.strip()
...
os.system(
    f'eww update calendar-selected-year={year} calendar-selected-month={month} calendar-selected-month-pretty="{selected_month_obj.strftime("%b").upper()}"'
)
