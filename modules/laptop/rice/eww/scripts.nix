...

{
  ...
  home.file.".config/eww/scripts/cal.sh" = {
    ...
    text = ''
      ...
      (while (true) do
          /home/${config.home.username}/.config/eww/scripts/cal.py
          sleep 1m
      done)
    '';
  };

  home.file.".config/eww/scripts/do-calendar-action" = {
    executable = true;
    text = ''
      #!/usr/bin/env ${pkgs.bash}/bin/bash
      ...
      reset() {
          ${pkgs.eww}/bin/eww update calendar-selected-year="$(date +%Y)" calendar-selected-month="$(date +%m)"
          "/home/${config.home.username}/.config/eww/scripts/cal.py"
      }

      month_down() {
          month="$(${pkgs.eww}/bin/eww get calendar-selected-month | sed 's/^0*//')"
          year="$(${pkgs.eww}/bin/eww get calendar-selected-year | sed 's/^0*//')"
          if [[ "$month" == 1 ]]; then
              month=12
              year=$((year - 1))
          else
          ...
          ${pkgs.eww}/bin/eww update calendar-selected-month="$month" calendar-selected-year="$year"
          "/home/${config.home.username}/.config/eww/scripts/cal.py"
      }
      ...
      year_down() {
          year="$(${pkgs.eww}/bin/eww get calendar-selected-year)"
          ${pkgs.eww}/bin/eww update calendar-selected-year=$((year - 1))
          "/home/${config.home.username}/.config/eww/scripts/cal.py"
      }
  ...
  home.file.".config/eww/scripts/cal.py" = {
    executable = true;
    text = ''
      ...
      year = subprocess.run(["${pkgs.eww}/bin/eww", "get", "calendar-selected-year"],
                          stdout=subprocess.PIPE, text=True).stdout.strip()
      month = subprocess.run(["${pkgs.eww}/bin/eww", "get", "calendar-selected-month"],
                          stdout=subprocess.PIPE, text=True).stdout.strip()
      ...
      os.system(f"${pkgs.eww}/bin/eww update calendar-selected-year={year} calendar-selected-month={month} calendar-selected-month-pretty=\"{selected_month_obj.strftime("%b").upper()}\"")

      calendar = get_month_calendar(int(year), int(month))
      # Get JSON format of calendar and escape double quotes
      os.system("${pkgs.eww}/bin/eww update calendar-json=\"%s\"" % (json.dumps(calendar).replace('"', '\\"')))
    '';
  };
  ...
  home.file.".config/eww/scripts/lyrics.py" = {
    executable = true;
    text = ''
  ...
  home.file.".config/eww/scripts/dock.py" = {
    executable = true;
    text = ''
      ...
      class Dock:
          ...
          def update_dock(self, dock_representation):
              ...
              os.system("${pkgs.eww}/bin/eww update dock-items-json='%s'" % (json.dumps(dock_representation)))
      ...
      with open("/home/${config.home.username}/.config/eww/scripts/dock-clients.json", "r", encoding="utf-8") as file:
          data = json.load(file)
