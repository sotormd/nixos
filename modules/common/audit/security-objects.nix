{
  ...
  security.audit.rules = [
    ...
    "-a always,exit -F path=/run/current-system/sw/bin/chage -F perm=x -F auid=0 -k privileged-chage"
    "-a always,exit -F path=/run/current-system/sw/bin/chcon -F perm=x -F auid=0 -k perm_mod"
  ];
}
